...
async function enableCourseMonitoring() {
  ...
  const { enabledCourses = {} } =
    await chrome.storage.local.get("enabledCourses");
...
async function scrapeD2LClasslist() {
  ...
  const previousStudents =
    (storageData.snapshots && storageData.snapshots[courseId]) || [];
  ...
  console.log(
    `LMS Monitor: Analyzing ${tables.length} tables on page for classlist matches...`,
  );
  ...
  for (const t of tables) {
    ...
    const hr =
      ...
      t.querySelector("tr[header]") ||
      t.querySelector("tr.d_gh") ||
      t.rows[0];
    if (!hr) continue;

    // Normalize headers by removing punctuation and collapsing whitespace
    const headers = Array.from(hr.cells).map((c) =>
      c.innerText
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]/g, " ")
    ...
    const isOrgIdHeader = (h) =>
      h.includes("org defined") ||
      h.includes("org id") ||
      (h.includes("org") && h.includes("id"));
    if (headers.some(isOrgIdHeader)) score += 3;

    if (
      headers.some(
        (h) =>
          h.includes("email") || h.includes("contact") || h.includes("mail"),
      )
    )
      score += 3;
    if (
      headers.some(
        (h) =>
          h.includes("role") || h.includes("group") || h.includes("status"),
      )
    )
      score += 2;
    ...
    if (score > maxScore && score >= 3) {
      // Require at least name column match
      maxScore = score;
      bestTable = t;
  ...
  console.log(
    `LMS Monitor: Selected table with score ${maxScore}. Mapped column headers:`,
    columnHeaders,
  );
  ...
  const tablesDebug = tables.map((t, idx) => {
    const hr =
      ...
      t.rows[0];
    const headers = hr
      ? Array.from(hr.cells).map((c) => c.innerText.trim().toLowerCase())
      : [];
    return {
      index: idx,
      classes: t.className,
  ...
  const isOrgIdHeader = (h) =>
    ...
    h.includes("org id") ||
    (h.includes("org") && h.includes("id"));
  const nameColIdx = columnHeaders.findIndex(
    (h) =>
      h.includes("name") ||
      h.includes("student") ||
      h.includes("user") ||
  ...
  const usernameColIdx = columnHeaders.findIndex(
    (h) =>
      h.includes("username") ||
      h.includes("login") ||
      h.includes("student id") ||
      (h.includes("id") && !isOrgIdHeader(h)),
  );
  const emailColIdx = columnHeaders.findIndex(
    (h) => h.includes("email") || h.includes("contact") || h.includes("mail"),
  );
  const roleColIdx = columnHeaders.findIndex(
    (h) => h.includes("role") || h.includes("group") || h.includes("status"),
  );
  ...
  for (const row of rows) {
    ...
    if (
      row === headerRow ||
      row.closest("thead") ||
      row.classList.contains("d_gh") ||
      row.hasAttribute("header")
    ) {
      continue;
    }
    ...
    if (nameColIdx !== -1 && row.cells[nameColIdx]) {
      ...
      const rawNameText = nameLink
        ? nameLink.innerText.trim()
        : row.cells[nameColIdx].innerText.trim();
    ...
    if (emailColIdx !== -1 && row.cells[emailColIdx]) {
      const emailLink =
        row.cells[emailColIdx].querySelector('a[href^="mailto:"]');
      email = emailLink
        ? emailLink.getAttribute("href").replace("mailto:", "").trim()
        : row.cells[emailColIdx].innerText.trim();
    }
    ...
    if (!name || name === "Name" || name === "Image" || name === "Learner") {
      ...
      const links = Array.from(
        row.querySelectorAll('a:not([href^="mailto:"])'),
      );
      const d2lLinks = links.filter(
        (l) => l.classList.contains("d2l-link") || l.href.includes("/d2l/"),
      );
      let rawNameText = "";
    ...
    if (!email) {
      const emailMatch = row.outerHTML.match(
    ...
    if (!orgId) {
      const prevMatch = previousStudents.find((s) => {
        ...
        if (email && s.email && s.email.toLowerCase() === email.toLowerCase())
          return true;
        if (name && s.name && s.name.toLowerCase() === name.toLowerCase())
          return true;
        return false;
      });
  ...
  console.log(
    `LMS Monitor: Scraped ${students.length} students from classlist.`,
  );
...
async function processClasslistChanges(courseId, courseName, currentStudents) {
  const {
    ...
    changeLog = [],
    courses = {},
  } = await chrome.storage.local.get(["snapshots", "changeLog", "courses"]);

  currentStudents = deduplicateStudentsByOrgId(currentStudents);
  ...
  const isFullClasslistScan =
    window.location.href.includes("classlist.d2l") ||
    (window.location.href.includes("playground.html") &&
      !mockUrl.includes("user_list_view.d2l"));
    ...
    for (const prev of previousStudents) {
      ...
      let matchIdx = currentStudents.findIndex((curr, index) => {
        ...
        if (
          curr.email &&
          prev.email &&
          curr.email.toLowerCase() !== prev.email.toLowerCase()
        )
          return false;
        ...
        if (
          ...
          return true;
        if (curr.name && prev.name && namesMatch(curr.name, prev.name))
          return true;
        return false;
      });
  ...
  if (detectedChanges.length > 0) {
    ...
    console.log(
      `LMS Monitor: Detected ${detectedChanges.length} changes!`,
      detectedChanges,
    );
  }
...
async function runScraperSequence() {
  ...
  updateWidget(
    "scanning",
    `Scanning classlist... (Attempt ${scrapeAttempts}/${maxScrapeAttempts})`,
  );
  ...
  try {
    ...
    updateWidget(
      "success",
      `Monitored successfully! ${studentCount} students tracked.`,
    );
    scrapeAttempts = 0; // reset
  } catch (err) {
    console.warn(
      `LMS Monitor (Attempt ${scrapeAttempts} failed):`,
      err.message,
    );
    ...
    if (scrapeAttempts < maxScrapeAttempts) {
      updateWidget(
        "scanning",
        `Retrying scan... (Attempt ${scrapeAttempts + 1}/${maxScrapeAttempts})`,
      );
      setTimeout(runScraperSequence, 2500); // Wait 2.5s and retry
    } else {
      updateWidget(
        "error",
        `Failed to monitor course. ${err.message || "Table not found"}.`,
      );
      scrapeAttempts = 0; // reset
    }
  }
